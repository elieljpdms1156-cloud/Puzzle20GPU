...
int main(int argc, char* argv[]) {
    ...
    // Carrega alvos do arquivo alvos.txt
    FILE* f = fopen("alvos.txt", "r");
    if (!f) { fprintf(stderr, "alvos.txt nao encontrado\n"); return 1; }
    char line[128];
    while (fgets(line, sizeof(line), f)) {
        char *p = strchr(line, '\n'); if(p) *p=0;
        p = strchr(line, '\r'); if(p) *p=0;
        if (strlen(line) == 40 && num_targets < MAX_TARGETS) {
            uint8_t *t = targets[num_targets];
            for (int i=0; i<20; i++) {
                unsigned int byte;
                sscanf(line + 2*i, "%2x", &byte);
                t[i] = (uint8_t)byte;
            }
    ...
    fclose(f);
    if (num_targets == 0) { fprintf(stderr, "Nenhum alvo carregado.\n"); return 1; }
    ...
    while(cmp128(chi,clo,0,hi)<=0) {
        int batch_count=0;
        for(int i=0; i<BATCH && cmp128(chi,clo,0,hi)<=0; i++) {
            size_t len=33;
        ...
        if(total % (BATCH*64)==0) {
            ...
            fprintf(stderr, "[stats] keys=%-12llu %.3f Mkeys/s\n", (unsigned long long)total, total/elapsed/1e6);
        }
    }
